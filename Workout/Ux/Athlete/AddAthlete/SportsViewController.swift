...
class SportsViewController: UITableViewController {
    //
    //  We use the following IDs to get the values to fetch from a table row.
    //
    private let COLORSWATCH         = 1000
    private let SPORTLABEL          = 1001
    
    // ---------------------------------------------------------------------------------------------
    // MARK: - Private Properties
    
    private let sports: [Sport]     = Sport.sports
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.sports.count > 0 {
            //
            //  Our rows will go from 0..number of sports in the array (the count). We use
            //  the indexpath row as an index into our sports array. That will get us the
            //  enumeration item. We then ask for the description to display in the table
            //  cell.
            //
            let colorSwatch: UIView = cell.viewWithTag(self.COLORSWATCH)!
            let sportLabel: UILabel = cell.viewWithTag(self.SPORTLABEL) as! UILabel
            
            sportLabel.text? = sports[indexPath.row].description()
            colorSwatch.backgroundColor? = sports[indexPath.row].color()
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //
        self.delegate?.setSport(sport: self.sports[indexPath.row])
    ...
    //
    func setSelectedSport(sport: Sport) {
        if let foundIndex = self.sports.index(where: { $0 == sport } ) {
