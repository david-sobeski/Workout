...
//
//  Thie file contains the implementation of our SkillViewController. It is a table view
//  that will list all the available skills.
//
...
class SkillViewController: UITableViewController {
    ...
    private let skills: [Skill]     = Skill.skills
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.skills.count > 0 {
            //
            //  Our rows will go from 0..number of skills in the array (the count). We use
            //  the indexpath row as an index into our skills array. That will get us the
            //  enumeration item. We then ask for the description to display in the table
            //  cell.
            //
            cell.textLabel?.text = skills[indexPath.row].description()
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //
        //  Call our delegate to tell it what skill was selected.
        //
        self.delegate?.setSkill(skill: self.skills[indexPath.row])
    ...
    //
    func setSelectedSkill(skill: Skill) {
        if let foundIndex = self.skills.index(where: { $0 == skill } ) {
